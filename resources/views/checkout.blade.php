...
<div id="midtrans-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden items-center justify-center p-6">
    <div class="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl">
        <div class="bg-slate-50 p-6 flex justify-between items-center border-b">
            ...
            <button onclick="hideMidtrans()" class="text-slate-400 hover:text-slate-600">✕</button>
        </div>
        <div class="p-8 text-center">
            ...
            <div class="mt-8 space-y-4">
                <button onclick="window.location.href='{{ route('ticket') }}'" class="w-full py-4 border-2 border-indigo-100 rounded-2xl font-bold hover:border-indigo-600 transition">
                    GoPay / QRIS
                </button>
            </div>
        </div>
...
<script>
    function showMidtrans() { document.getElementById('midtrans-overlay').classList.replace('hidden', 'flex'); }
    function hideMidtrans() { document.getElementById('midtrans-overlay').classList.replace('flex', 'hidden'); }
</script>
@endsection